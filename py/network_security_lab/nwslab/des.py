# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 18:33:12 2016
DES - Data Encryption Standard Algo Implementation
@author: zerone
"""
...
def get_key(pc1): # Gets 64 bit key from user and returns 56 bit key
    key = str(raw_input('Enter alphanumeric key of length 8: '))
    binary_key64 = '' # Stores the binary of entered key
    for i in range(8):
        binary_key64+=('{0:08b}'.format(ord(key[i])))
    
    # Generate 56 bit key by applying permutation choice 1
    binary_key56=''
    for val in pc1:
...
def generate_subkeys(binary_key56,left_rotations,pc2): # Generate 16 subkeys
    ...
    subkeys48 = [] # Stores all the 48 bit 16 subkeys 
    
    for shifts in left_rotations: # Account for left bit rotations in subkeys
...
# Step 2: Encode 64 bit block message data
    
def initial_permutation(message,ip): # Generate the initial permutation of plaintext
    binary_message64=''
    for i in range(8):
...
def expansion32to48(right_half32, expansion_table): # Expands the 32 bit right half to 48 bit right_half
    right_half48=''
...
def f_function(right_half48,subkey48,s_boxes,s_box_p): # Does xor of right_half48 with round subkey48 and applies 8 s boxes
                                               # to convert 48 bit xorred value to 32 bit output
    key = [int(subkey48[i]) for i in range(48)] # Convert the subkey48 from string to integer list
    message = [int(right_half48[i]) for i in range(48)] # Convert the right_half48 from string to integer list
    xorred_output = [key[i] ^ message[i] for i in range(48)] # xor the subkey48 with right_half48
    s_index = 0
    s_values = [] #Stores the 8 s box values, each of 4 bits number (0 to 15)
    ...
    # Create the 32 bit message as the output of f_function
    right_half32 = ''
    for val in s_values:
        right_half32+='{0:04b}'.format(val)
    
    # Do the permutation on right_half32 bit output of s box
...
def string_xor(left_half32,permuted_right_half32): # Does the xor operation on left_half32 bit message and 
                                                   # permuted_right_half32 output from f_function
    xorred_output=''.join(str(int(left_half32[i]) ^ int(permuted_right_half32[i])) for i in range(32))
    return xorred_output # String of xorred values of left_half32 and permuted_right_half32
    

def encryption_rounds(permuted_message64,subkeys48,expansion_table,s_boxes,s_box_p,final_p):
    left_half32 = permuted_message64[0:32] # Get L0 of 32 bit in length
    right_half32 = permuted_message64[32:64] # Get R0 of 32 bit in length
...
if __name__ == '__main__':
    ...
    message = str(raw_input('Enter alphanumeric message of length 8: ')) # Get the message from user
