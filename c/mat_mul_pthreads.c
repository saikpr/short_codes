...
int main()
{
	...
	xycood xyvar[10000];
	int threadcount,threadmaxcount,i,j,it[10000],til;
	float gpvar;
	clock_t begin, end;
	double time_spent;


	printf("Enter the Max Number of threads :");
	scanf("%d",&threadmaxcount);
	printf("\nEnter the Size N:");
	scanf("%d",&n);
	...
	*/
	...
	{	gpvar=(n*n)/(float)threadcount;
		...
		*/
		...
		printf("Elapsed: %f second with %d Thread count\n", time_spent,threadcount);
